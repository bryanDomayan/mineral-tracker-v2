    ...
    return (
        <aside className='w-64 h-screen bg-white fixed top-0 p-4 left-0'>
            <div className='bg-teal-800/10 p-2 rounded-lg'>
                <h2 className='text-lg tracking-tight capitalize leading-tight font-bold text-gray-600 text-center'>Water consumption monitoring system</h2>
            </div>
            <ul className='my-6'>
                {navList.map((d, i) => (
                    <li key={i}>
                        <Link 
                            href={d.path} 
                            className={`flex gap-2 items-center rounded-lg hover:bg-teal-800/10 p-3 px-4 text-gray-600 ${pathname == d.path ? 'text-teal-500 pointer-events-none' : ''}`}
                        >
                            <d.icon size={20} />
                            {d.name}
                        </Link>
